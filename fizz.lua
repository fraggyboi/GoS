...
local capspress = KeyIsDown(0x14);  --checks if a specific key is pressed (0x14 is CapsLock; check "virtual key codes" in google)
 ...
 if not KeyIsDown(0x20) then return end
    if ValidTarget(unit, GetRange(myHero)) then
      if walk then
        ...
        else if Ready(_R) and tsDistance < 1000 then
			afterCombo = false
			CastR(ts.target)
			Rcheck = os.clock()
		end
		if CanAttack() and ValidTarget(ts.target) and not BeforeAttack(ts.target) and os.clock() < Rcheck + 1 and ts.target.health > cmbo2 then
		...
		if CanAttack() and ValidTarget(ts.target) and not BeforeAttack(ts.target) and os.clock() < Wcheck + 1 and ts.target.health > cmbo2 then
			AAcheckY = os.clock()
		...
		if os.clock() < Qcheck + 1 and ts.target.health > cmbo2 and CanAttack() and ValidTarget(ts.target) and not BeforeAttack(ts.target) then
			AAcheckZ = os.clock()
			Attack(ts.target)
		end
